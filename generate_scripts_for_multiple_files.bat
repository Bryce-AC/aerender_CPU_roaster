...
:loop
	...
	echo "C:\Program Files\Adobe\Adobe After Effects 2020\Support Files\aerender.exe" -project "%file_path%" -s %start% -e %end% -comp "render" -rqindex 1 -sound ON -output "%~d0%~p0render_output\out%instances%.mov" > render_scripts\render_%instances%.bat
